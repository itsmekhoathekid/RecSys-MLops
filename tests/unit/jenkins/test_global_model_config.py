...
def test_global_config_and_agents_use_native_modelconfig_reference():
    ...
    for chart in (
        ...
        assert (
            ...
            not in agent["spec"]["declarative"]["systemMessage"]
        )


def test_global_config_deploys_before_each_consumer():
    plan = create_release_plan(
        ["context_agent", "recommendation_agent", "coordinator_agent"],
        commit=TEST_COMMIT,
    )
...
def test_global_values_change_selects_all_snapshot_consumers():
    ...
    assert {"context_agent", "recommendation_agent", "coordinator_agent"} <= set(
