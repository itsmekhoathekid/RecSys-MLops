...
sources and selected unit-test oracles into an isolated conventional workspace.
Nothing under ``apps/`` is rewritten and only JSON/text reports are retained.
"""
...
REPORT_ROOT = ROOT / "tests" / "unit" / "api_serving" / "reports"
BAD_STATUSES = {
    "not checked",
    "no tests",
    "suspicious",
...
SERVICE_CONFIG = {
    "inference": {
        ...
        "unit_test_dir": "inference_api",
        "mutation_test_files": ["test_mutation_oracles.py"],
    ...
    "online-feature": {
        ...
        "unit_test_dir": "online_feature_api",
        "mutation_test_files": ["test_mutation_oracles.py"],
        "prefixes": [
            "recsys_online_feature_api.",
            "recsys_serving_common.contracts.",
    ...
    "rag": {
        ...
        "unit_test_dir": "rag_api",
        "mutation_test_files": [
            "test_mutation_oracles.py",
            "test_retrieval.py",
            "test_chunk_lookup.py",
        ],
        # Every unit test is still executed by the clean preflight below.
        # Mutmut 3.6 runs coverage and stats in one interpreter and partially
        # unloads modules between them, so the FastAPI TestClient lifecycle
        # tests are not safe in that second import. The direct batching unit
        # test can also block on intentionally broken synchronization mutants.
        # The selected unit-test oracles cover both areas with deadline-safe
        # behavior and avoid the unsafe second import.
    },
}
SOURCE_PACKAGES = {
...
def copy_workspace(workspace: Path, service: str) -> Path:
    src = workspace / "src"
    src.mkdir()
    for package, source in SOURCE_PACKAGES.items():
        shutil.copytree(source, src / package)
    config = SERVICE_CONFIG[service]
    unit_tests = ROOT / "tests" / "unit" / "api_serving" / config["unit_test_dir"]
    unit_preflight = workspace / "unit_preflight"
    shutil.copytree(
        unit_tests,
        unit_preflight,
    ...
    selected_tests = workspace / "tests"
    selected_tests.mkdir()
    for support_file in ("__init__.py", "conftest.py"):
        source = unit_tests / support_file
        if source.exists():
            shutil.copy2(source, selected_tests / support_file)
    for test_file in config["mutation_test_files"]:
        shutil.copy2(unit_tests / test_file, selected_tests / test_file)
    shutil.copy2(ROOT / "pyproject.toml", workspace / "pyproject.toml")
    return unit_preflight
...
def run_clean_unit_preflight(
    workspace: Path, test_dir: Path
) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env.setdefault("RECSYS_OTEL_ENABLED", "0")
    env["PYTHONPATH"] = str(workspace / "src")
...
def run_service(service: str, output_dir: Path, max_children: int) -> bool:
    with tempfile.TemporaryDirectory(prefix=f"recsys-mutmut-{service}-") as directory:
        ...
        unit_preflight = copy_workspace(workspace, service)
        preflight_output = ""
        preflight = run_clean_unit_preflight(workspace, unit_preflight)
        preflight_output = preflight.stdout + preflight.stderr
        if preflight.returncode != 0:
            print("Clean unit preflight failed:\n" + preflight_output, file=sys.stderr)
            return False
        config = SERVICE_CONFIG[service]
        result = command(
            workspace,
