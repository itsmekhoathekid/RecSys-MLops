...
DAGS = (
    ...
    "recsys_rag_item_reconciliation.py",
)
...
def test_data_platform_dags_have_no_datahub_runtime_coupling():
    ...
    analytics = Path("apps/analytics/orchestration/airflow/dags/recsys_analytics_daily.py").read_text()
    assert "metadata." not in analytics
...
def test_task_dependencies_and_rag_runtime_parameters_are_preserved():
    root = Path("apps/data-platform/src/orchestration/airflow/dags")
    assert "ingest_stage >> optimize_stage >> validate_stage" in (root / DAGS[0]).read_text()
