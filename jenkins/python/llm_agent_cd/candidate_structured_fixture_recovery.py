"""Recover the candidate after a control-only A2A request-string ambiguity in fixture v3."""
import argparse
...
REVIEW="reviewed-structured-request-fixture-v4-recovery-v1"
EXPERIMENT="wf-63cfe90471f6f49d12dbce16ecd8a9e0"
...
FIXTURE_SHA="5f6e82db1cf670c45aee301cef53349ac2a38d1aedd72313ece8ef78656814ed"
...
def validate_recovery(state, evidence):
    if (state.get('phase')!='ROLLED_BACK' or state.get('experiment_id')!=EXPERIMENT
            ...
            or state.get('cases')):
        raise ValueError('exact structured-fixture rollback state required')
    ...
    expected={('live_test',CONTROL,'FAIL','child runtime error',True,True):1}
    if evidence.get('invocations')!=expected or evidence.get('submitted')!=1:
        raise ValueError('only reviewed single control-failure/no-candidate window may be recovered')
    ...
    if len(events)!=1: raise ValueError('candidate enabled without exact structured-fixture recovery evidence')
    return 'recovered'


def main():
    p=argparse.ArgumentParser();p.add_argument('--image',required=True);args=p.parse_args()
    if args.image!=REVIEW: raise ValueError('unreviewed structured-fixture recovery')
    ...
    check()
    fixture=Path('configs/llm-ab/workflow-cases-v4.json').read_bytes()
    provenance=json.loads(Path('configs/llm-ab/workflow-cases-v4.provenance.json').read_text())
    if hashlib.sha256(fixture).hexdigest()!=FIXTURE_SHA or provenance.get('fixture_file_sha256')!=FIXTURE_SHA:
        raise ValueError('reviewed fixture v4 checksum mismatch')
    ...
    record={'stage':'candidate_structured_fixture_recovery','review':REVIEW,'failed_experiment':EXPERIMENT,
      'release_id':RELEASE,'counts':{'live_test':1,'control_fail':1,'candidate_hold':0,
      'synthetic':0,'offline':6,'offline_confirmed':6},'fixture_sha256':FIXTURE_SHA,
      'reason':'fixture v3 used natural-language result limit although the nested A2A request string required explicit top_k',
      'build_url':os.environ['BUILD_URL'],'at':time.time(),'inference_requests':0}
    key='workflow/recoveries/'+EXPERIMENT+'/structured-request-fixture-v4.json'
    if status=='pending':
        ...
        for field in ('stage','review','failed_experiment','release_id','counts','fixture_sha256','reason','inference_requests'):
            if persisted.get(field)!=record.get(field):raise ValueError('structured-fixture recovery evidence mismatch')
        record=persisted
    after,_=store.read()
    if RELEASE in after.get('disabled',[]) or after['champion']!=state['champion']:
        raise ValueError('structured-fixture recovery state verification failed')
