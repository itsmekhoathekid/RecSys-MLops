"""Path rules and CLI for deterministic monorepo component change detection."""
...
def classify_jenkins(flags: dict[str, bool], path: str) -> None:
    ...
    elif path.startswith(("jenkins/scripts/ci/serving", "jenkins/scripts/build/serving", "jenkins/scripts/deploy/serving", "jenkins/scripts/test/serving")):
        mark(flags, "API", "KSERVE")
    elif path.startswith(("jenkins/scripts/ci/analytics", "jenkins/scripts/build/analytics", "jenkins/scripts/deploy/analytics", "jenkins/scripts/test/analytics")):
        mark(flags, "ANALYTICS")
...
def apply_path_rules(flags: dict[str, bool], normalized: str) -> None:
    ...
    if normalized in {"pyproject.toml", "uv.lock", "requirements.txt"}:
        ...
        mark(flags, "STREAM_OFFLINE", "STREAM_ONLINE")
    elif normalized == "apps/data-platform/Dockerfile.data-ingestion":
        mark(flags, "DP1")
    elif normalized == "apps/data-platform/Dockerfile.feature-store":
        mark(flags, "MATERIALIZE", "DP3")
    elif normalized == "apps/data-platform/Dockerfile.drift-retrain":
        mark(flags, "DRIFT")
