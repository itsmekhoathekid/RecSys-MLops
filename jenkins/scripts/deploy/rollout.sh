...
reconcile_rollout_jenkins_jobs() {
  local values_file="$1"
  local namespace="$2"
  local jenkins_url="${JENKINS_URL:-http://recsys-jenkins.${namespace}.svc.cluster.local:8080}"
  ...
  helm template recsys-ci infra/helm/recsys-ci \
    ...
    | kubectl apply -f -
  mkdir -p "${seed_dir}"
  kubectl get configmap recsys-jenkins-init -n "${namespace}" \
    -o 'jsonpath={.data.zz-seed-cicd-views\.groovy}' >"${seed_script}"
  username="$(kubectl get secret "${admin_secret}" -n "${namespace}" -o 'jsonpath={.data.username}' | base64 -d)"
  password="$(kubectl get secret "${admin_secret}" -n "${namespace}" -o 'jsonpath={.data.password}' | base64 -d)"
...
deploy_rollout_watcher() {
  ...
  local watcher_image
  local values_file
  watcher_image="$(resolve_release_image recsys-mlops-training)"
  values_file="${ROLLOUT_CI_VALUES_FILE:-infra/helm/recsys-ci/values-gke.yaml}"
  reconcile_rollout_jenkins_jobs "${values_file}" "${namespace}"
  helm template recsys-ci infra/helm/recsys-ci \
    --namespace "${namespace}" \
    -f "${values_file}" \
    --set "namespace.name=${namespace}" \
  ...
  verify_and_wait_workload \
    deployment recsys-model-rollout-watcher "${namespace}" "${watcher_image}"
}
