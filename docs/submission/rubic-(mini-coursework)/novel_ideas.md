...
flowchart LR
    ...
    DATAHUB["DataHub static batch lineage"]
...
| `DATAHUB` — Static dataset lineage | [`governance_catalog.py`](../../../apps/data-platform/src/metadata/governance_catalog.py) declares the six Silver and two Bronze inputs for their matching analytics staging outputs. The independent catalog sync publishes these relationships without coupling Airflow or dbt execution to DataHub. | Records the governed cross-catalog copy as stable dataset metadata. |
