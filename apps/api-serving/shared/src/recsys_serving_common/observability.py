...
SERVICE_NAME = os.getenv("OTEL_SERVICE_NAME", "recsys-service")
...
class MetricsStore:
    ...
    summaries: dict[str, dict[tuple[tuple[str, str], ...], dict[str, float]]] = field(
        default_factory=dict
    )
    histograms: dict[str, dict[tuple[tuple[str, str], ...], dict[str, Any]]] = field(
        default_factory=dict
    )
    ...
    def observe(
        ...
        bucket = series.setdefault(
            _label_key(labels), {"count": 0.0, "sum": 0.0, "max": 0.0}
        )
        bucket["count"] += 1.0
        bucket["sum"] += value
    ...
    def observe_histogram(
        ...
        bucket = series.setdefault(
            key,
            {
                "buckets": {boundary: 0.0 for boundary in buckets},
                "count": 0.0,
                "sum": 0.0,
            },
        )
        ...
        for name in sorted(self.histograms):
            ...
            for labels, values in sorted(self.histograms[name].items()):
                for boundary, count in sorted(values["buckets"].items()):
                    ...
                    lines.append(
                        f"{name}_bucket{_format_labels(_label_key(bucket_labels))} {count}"
                    )
...
@contextmanager
def timed_operation(
    metric: str, labels: dict[str, str] | None = None
) -> Iterator[None]:
...
def observe_triton(
    ...
    METRICS.observe(
        "recsys_api_triton_inference_duration_seconds", duration_seconds, metric_labels
    )
    if error:
