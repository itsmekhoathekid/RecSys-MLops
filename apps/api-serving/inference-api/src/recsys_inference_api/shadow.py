...
from recsys_serving_common.observability import LATENCY_BUCKETS, METRICS
...
class ShadowRunner:
    ...
    def submit(self, route: TritonRoute | None, payload: dict[str, np.ndarray]) -> bool:
        if route is None:
            return False
        labels = ab_labels(
            "shadow_candidate", route.model_version, route.ab_experiment_id
        )
        if self.pending_count >= self.max_pending:
            METRICS.inc(
                "recsys_api_shadow_inferences_total",
                labels={**labels, "status": "dropped"},
            )
            METRICS.set_gauge(
                "recsys_api_shadow_queue_depth", self.pending_count, labels=labels
            )
            return False
    ...
    async def _run(
        ...
        try:
            ...
            status = (
                "timeout" if inference_duration >= self.timeout_seconds else "success"
            )
            if status == "success" and scores:
                METRICS.observe(
                    "recsys_api_shadow_score_mean",
                    float(np.mean(scores)),
                    labels=labels,
                )
                METRICS.set_gauge(
                    "recsys_api_shadow_score_max", float(np.max(scores)), labels=labels
                )
