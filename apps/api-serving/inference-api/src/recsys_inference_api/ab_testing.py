...
class TritonABRouter:
    def __init__(
        ...
        self.shadow_enabled = (
            shadow_enabled and candidate_ranker is not None and not self.enabled
        )
        ...
        rollout_mode = (
            "ab" if self.enabled else "shadow" if self.shadow_enabled else "stable"
        )
        METRICS.set_gauge(
            ...
            labels={
                ...
                "candidate_model_version": (
                    self.candidate_model_version if self.candidate_ranker else "none"
                ),
                "candidate_weight_percent": str(
                    self.candidate_weight_percent if self.enabled else 0
                ),
            },
        )
    ...
    def shadow_route(self, user_id: int) -> TritonRoute | None:
        if (
            ...
            or self.candidate_ranker is None
            or self.shadow_sample_percent <= 0
        ):
            return None
        if (
            self.shadow_sample_percent < 100
            and self._bucket(user_id, "shadow") >= self.shadow_sample_percent
        ):
            return None
        return TritonRoute(
    ...
    def route(self, user_id: int) -> TritonRoute:
        ...
        if variant == "candidate" and self.candidate_ranker is not None:
            ...
            route = TritonRoute(
                ...
                ab_experiment_id=(
                    self.experiment_id
                    if (self.enabled or self.shadow_enabled)
                    else None
                ),
            )
        METRICS.inc(
