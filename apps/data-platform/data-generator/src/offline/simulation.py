...
from offline.problems import DataSkewProblem, HighCardinalityProblem
from offline.payload_hash import event_payload_hash
from randomness import utc_datetime, weighted_index
...
class RecsysSimulation:
    def __init__(self, config: GeneratorConfig):
        ...
        self.high_cardinality = HighCardinalityProblem(config.seed)
        self.skew = DataSkewProblem(
            self.rng, config.distribution, config.entities.n_categories
        )
    ...
    def generate(self) -> GeneratedData:
        ...
        challenge_multiplier = 1 + self.config.challenges.duplicate_event_rate
        clean_target = int(
    ...
    def _generate_users(self) -> tuple[list[User], list[UserPreference]]:
        ...
        city_weights = self.skew.city_weights()
        ...
        for user_id in range(1, self.config.entities.n_users + 1):
            ...
            category = self.skew.category_id()
            ...
            while len(preference_categories) < self.config.entities.preferences_per_user:
                candidate = self.skew.category_id()
    ...
    def _generate_products(
        ...
        for product_id in range(1, self.config.entities.n_products + 1):
            category_id = self.skew.category_id()
            ...
            popularity = self.skew.popularity_weight()
    ...
    def _generate_session(
        ...
        session_id = self.high_cardinality.next_id("session")
        ...
        for _ in range(request_count):
            request_id = self.high_cardinality.next_id("request")
            surface = str(
            ...
            candidates = self.skew.select_candidates(user, products, candidate_count)
            request_impressions: list[Impression] = []

            for rank_position, product in enumerate(candidates, start=1):
                impression_id = self.high_cardinality.next_id("impression")
                ...
                for state in states:
                    ...
                    if state == SessionState.PURCHASE:
                        order_id = self.high_cardinality.next_id("order")
    ...
    def _make_event(
        ...
        event = BehaviorEvent(
            event_id=self.high_cardinality.next_id("event"),
    ...
    def _make_order(
        ...
        item = OrderItem(
            order_item_id=self.high_cardinality.next_id("order_item"),
            order_id=order_id,
            product_id=product.product_id,
            quantity=quantity,
