...
def main() -> int:
    if not Path("apps/data-platform/feature-store/feature_repo/feature_store.yaml").exists():
        print({"passed": False, "missing": ["apps/data-platform/feature-store/feature_repo/feature_store.yaml"]})
        return 1

    offline_root = os.getenv("FEAST_OFFLINE_ROOT", "data_platform/output/feature_store/offline")
    require_offline = os.getenv("FEAST_REQUIRE_OFFLINE", "").lower() in {"1", "true", "yes"}
    ...
    if missing:
        if not require_offline:
            print({"passed": True, "mode": "metadata_only", "missing_offline": missing})
            return 0
