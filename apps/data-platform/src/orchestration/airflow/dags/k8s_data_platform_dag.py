...
if DAG is not None:
    with DAG(
        ...
        sync_offline_to_online_store = pod_task(
            ...
            DATAFLOW_IMAGE,
            "python -m feature_store.offline_to_online_sync",
        )
        evidently_feature_drift = pod_task(
            "evidently_feature_drift",
            DATAFLOW_IMAGE,
            "python -m validate.evidently_feature_drift "
            "--report-path s3://recsys-lake/monitoring/evidently/feature_drift.json",
        )
        datahub_ingest = pod_task(
            "datahub_ingest",
            DATAFLOW_IMAGE,
            "echo 'DataHub ingestion placeholder: run metadata ingestion for Airflow, dbt, Kafka, Postgres, MinIO, Feast'",
        )
