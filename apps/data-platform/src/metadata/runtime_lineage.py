...
def _dataset_identity(urn: str) -> tuple[str, str]:
    ...
    if env != ENV:
        raise ValueError(
            f"Runtime lineage dataset environment {env!r} does not match DataHub OpenLineage environment {ENV!r}: {urn}"
        )
    # DataHub's native OpenLineage mapper treats a bare namespace as the data platform
    # and preserves name verbatim, yielding the exact catalog URN above.
