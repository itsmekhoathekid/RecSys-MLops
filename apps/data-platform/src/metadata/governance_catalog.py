...
ANALYTICS_SILVER_TABLES = (
    "clean_behavior_events", "clean_impressions", "clean_recommendation_requests",
    "product_scd", "users", "products",
)
...
def _dataset(
    urn: str, name: str, description: str, product: str,
    schema: tuple[SchemaColumn, ...], *, primary_keys: tuple[str, ...] = (),
    upstreams: tuple[str, ...] = (), extra_tags: tuple[str, ...] = (),
    custom_properties: Mapping[str, str] | None = None,
) -> DatasetSpec:
    return DatasetSpec(
        urn=urn, name=name, description=description, schema=schema,
        primary_keys=primary_keys, tags=(product, "BatchPipeline", *extra_tags),
        upstreams=upstreams,
...
def dp3_product() -> DataProductSpec:
    ...
    for table in DP3_ICEBERG_TABLES:
        if table == "ml_bst_training":
            upstreams = tuple(ICEBERG_FEATURE_URNS[item] for item in (
        ...
        iceberg.append(_dataset(
            ICEBERG_FEATURE_URNS[table], f"recsys_features.feature_store.{table}",
            "DP3 batch feature dataset stored in Iceberg.", "DP3",
            feature_schema(table), primary_keys=FEATURE_PRIMARY_KEYS[table],
            upstreams=upstreams,
        ))
    postgres = tuple(
        _dataset(
            POSTGRES_FEATURE_URNS[table], f"feature-postgres.feature_store.{table}",
            "Feast PostgreSQL offline dataset exported from the matching Iceberg feature.",
            "DP3", feature_schema(table), primary_keys=FEATURE_PRIMARY_KEYS[table],
    ...
    redis = tuple(
        _dataset(
            REDIS_FEATURE_URNS[table], f"redis_online.{table}",
            "Feast Redis online feature dataset materialized from PostgreSQL.",
            "DP3", feature_schema(table), primary_keys=FEATURE_PRIMARY_KEYS[table][:1],
            upstreams=(POSTGRES_FEATURE_URNS[table],),
        )
        for table in FEATURE_TABLES
    )
    return DataProductSpec(
        "DP3", "DP3 Batch Feature Store",
        "Silver data transformed into Iceberg, PostgreSQL, and Redis feature datasets.",
        ("DP3", "BatchPipeline"), tuple(iceberg) + postgres + redis,
    )


def rag_product() -> DataProductSpec:
    common = {"embedding_model": "intfloat/multilingual-e5-small", "embedding_dimension": "384"}
    datasets = (
        _dataset(
            RAG_RAW_DOCUMENTS_URN, "recsys-lakehouse.raw.rag_item_documents",
            "Canonical generated item documents.", "RAG_ITEMS", rag_schema("raw"),
            primary_keys=("item_id",), custom_properties=common,
        ),
        _dataset(
            RAG_SILVER_CHUNKS_URN, "recsys-lakehouse.silver.rag_item_chunks",
            "Structure-aware semantic item chunks.", "RAG_ITEMS", rag_schema("silver"),
            primary_keys=("chunk_id",), upstreams=(RAG_RAW_DOCUMENTS_URN,),
            extra_tags=("SemanticChunking",), custom_properties=common,
        ),
        _dataset(
            RAG_GOLD_EMBEDDINGS_URN, "recsys-lakehouse.gold.rag_item_embeddings",
            "Normalized multilingual item embeddings.", "RAG_ITEMS", rag_schema("gold"),
            primary_keys=("chunk_id",), upstreams=(RAG_SILVER_CHUNKS_URN,),
            extra_tags=("Embedding",), custom_properties=common,
        ),
        *tuple(
            _dataset(
                urn, f"recsys_rag.rag_item_chunks_{slot}",
                f"Milvus {slot} collection used for atomic index promotion.",
                "RAG_ITEMS", rag_schema("milvus"), primary_keys=("chunk_id",),
                upstreams=(RAG_GOLD_EMBEDDINGS_URN,), extra_tags=("VectorIndex",),
                custom_properties={**common, "slot": slot},
            )
            for slot, urn in RAG_MILVUS_URNS.items()
        ),
        _dataset(
            RAG_ACTIVE_POINTER_URN, "recsys-lakehouse.gold.rag_item_embeddings._active",
            "Atomic pointer selecting the active Milvus index slot.",
            "RAG_ITEMS", rag_schema("pointer"), primary_keys=("active_slot",),
            upstreams=tuple(RAG_MILVUS_URNS.values()), extra_tags=("VectorIndex",),
            custom_properties=common,
        ),
    )
    return DataProductSpec(
        "RAG_ITEMS", "RAG Item Index",
        "Item documents, chunks, embeddings, and blue/green Milvus indexes.",
        ("RAG_ITEMS", "BatchPipeline"), datasets,
    )
...
def analytics_product() -> DataProductSpec:
    datasets = []
    for table in ANALYTICS_SILVER_TABLES + ANALYTICS_BRONZE_TABLES:
        source = SILVER_URNS[table] if table in ANALYTICS_SILVER_TABLES else BRONZE_URNS[table]
        datasets.append(_dataset(
            ANALYTICS_STAGING_URNS[table], f"analytics.staging.{table}",
            "Analytics staging copy of the canonical lakehouse dataset.",
            "ANALYTICS", analytics_schema(table), upstreams=(source,),
        ))
    return DataProductSpec(
        "ANALYTICS", "Analytics Staging",
        "Batch-synchronized lakehouse datasets exposed to analytics workloads.",
        ("ANALYTICS", "BatchPipeline"), tuple(datasets),
    )


def catalog_products() -> tuple[DataProductSpec, ...]:
    return (dp1_product(), dp2_product(), dp3_product(), rag_product(), analytics_product())


def validate_catalog(products: tuple[DataProductSpec, ...] | None = None) -> dict[str, object]:
    ...
    for dataset in datasets:
        ...
        if missing_keys:
            errors.append(f"Unknown primary keys for {dataset.urn}: {sorted(missing_keys)}")
    if errors:
