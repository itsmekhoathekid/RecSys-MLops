...
class LateArrivalMetricCounters:
    ...
    def __init__(self, metric_group: Any):
        ...
        self.accepted_late_events_total = metric_group.counter(
...
def late_arrival_metrics(
    event: dict[str, Any], allowed_lateness_seconds: int
) -> tuple[float, bool]:
...
def event_time_status(
    ...
    is_too_late = (
        current_watermark_ms >= window_end_ms + max(0, allowed_lateness_seconds) * 1000
    )
