...
class UserAggregateState:
    events_by_user: dict[int, deque[dict[str, Any]]] = field(
    ...
    def update(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        h30 = [
            row
            for row in rows
            if parse_event_time(row["event_timestamp"]) > ts - timedelta(minutes=30)
        ]
        h24 = [
            row
            for row in rows
            if parse_event_time(row["event_timestamp"]) > ts - timedelta(hours=24)
        ]
        viewed_prices = [
        ...
        return {
            ...
            "avg_viewed_price_7d": float(sum(viewed_prices) / len(viewed_prices))
            if viewed_prices
            else 0.0,
            "cart_to_purchase_ratio_7d": float(purchases_7d / carts_7d)
            if carts_7d
            else 0.0,
