resource "helm_release" "recsys_observability" {
  ...
  set {
    name  = "chartRevision"
    value = local.observability_chart_revision
  }
  ...
  depends_on = [
    ...
    kubernetes_secret_v1.langfuse_otel,
...
resource "helm_release" "recsys_online_feature_api" {
  count = var.config.deploy_serving ? 1 : 0
...
resource "helm_release" "recsys_serving" {
  count = var.config.deploy_model_serving ? 1 : 0
...
resource "helm_release" "recsys_ray_gpu" {
  count = var.config.deploy_ray_job ? 1 : 0
  ...
  values = [
    file(var.config.enable_gpu_pool ? "${local.helm_dir}/ray-cluster/values-gcp-gpu.yaml" : "${local.helm_dir}/ray-cluster/values-gcp-cpu.yaml"),
  ]
...
resource "helm_release" "recsys_gateway" {
  ...
  set {
    name  = "gateway.domain"
    value = var.config.gateway_domain
  }

  set {
    name  = "chartRevision"
    value = local.gateway_chart_revision
  }
  ...
  set {
    ...
    value = "api.${var.config.gateway_domain}"
  }
  ...
  set {
    name  = "grafana.host"
    value = "metrics.${var.config.gateway_domain}"
  }

  set {
    name  = "logs.host"
    value = "logs.${var.config.gateway_domain}"
  }
  ...
  set {
    name  = "traces.host"
    value = "traces.${var.config.gateway_domain}"
  }
  ...
  set {
    name  = "agents.host"
    value = "agents.${var.config.gateway_domain}"
  }

  set {
    name  = "agents.upstreamHost"
    value = "kagent-ui.kagent.svc.cluster.local"
  }
  ...
  set {
    name  = "registry.host"
    value = "registry.${var.config.gateway_domain}"
  }

  set {
    name  = "registry.upstreamHost"
    value = "agentregistry.agentregistry.svc.cluster.local"
  }
  ...
  set {
    name  = "rag.host"
    value = "rag.${var.config.gateway_domain}"
  }

  set {
    name  = "rag.rootRedirect.enabled"
    value = "true"
  }

  set {
    name  = "rag.rootRedirect.path"
    value = "/docs"
  }

  set {
    name  = "rag.upstreamHost"
    value = "recsys-rag-api.api-serving.svc.cluster.local"
  }

  set {
    name  = "tls.enabled"
    value = tostring(var.config.gateway_tls_enabled)
  }

  set {
    name  = "tls.clusterIssuerName"
    value = var.config.gateway_tls_cluster_issuer
  }

  set {
    name  = "tls.issuer.create"
    value = tostring(var.config.gateway_tls_issuer_create)
  }
  ...
  set {
    ...
    value = var.config.gateway_tls_issuer_email
  }

  set {
    name  = "tls.issuer.server"
    value = var.config.gateway_tls_issuer_server
  }

  set {
    name  = "tls.issuer.privateKeySecretName"
    value = "${var.config.gateway_tls_cluster_issuer}-account-key"
  }
...
resource "helm_release" "recsys_security" {
  ...
  set {
    name  = "externalSecrets.gatewayKagent.enabled"
    value = "true"
  }

  set {
    name  = "externalSecrets.gatewayAgentRegistry.enabled"
    value = "true"
  }
