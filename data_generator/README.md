...
```bash
uv run python -m data_generator.cli generate \
  --config config/data_generator_test.yaml

uv run python -m data_generator.cli validate \
  --config config/data_generator_test.yaml
```

Output is written to `data_generator/output/<run_id>/`. Time-series tables use
...
`config/data_generator_drift.yaml` enables Scenario A over 150 historical days:
...
```bash
uv run python -m data_generator.cli generate \
  --config config/data_generator_drift.yaml
```
